...
public abstract class AbstractBeanDefinitionReader implements EnvironmentCapable, BeanDefinitionReader {
     ...
     * <p>
     * 如果传入的bean工厂不只是实现BeanDefinitionRegistry
     * 接口，但也ResourceLoader接口，它将作为默认使用
     * 还有ResourceLoader。通常是这样的
     ...
     *
     * 如果给定一个简单的BeanDefinitionRegistry，默认的ResourceLoader将是a
     * {@link org.springframework.core.io.support.PathMatchingResourcePatternResolver}。
     * 如果传入的bean工厂也实现了{@link EnvironmentCapable} its
     * 本读者将使用环境。否则，读取器将初始化和
     * 使用{@link StandardEnvironment}。所有的ApplicationContext实现都是
     * 环保的，而正常的BeanFactory实现不是。
     *
