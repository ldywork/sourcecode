...
/**
 * The root interface for accessing a Spring bean container.访问Spring bean容器的根接口。 This is
 * the basic client view of a bean container;这是一个bean容器的基本客户端视图; further interfaces such
 * as {@link ListableBeanFactory} and
 * {@link org.springframework.beans.factory.config.ConfigurableBeanFactory} are available
 * for specific purposes. 进一步的接口，如{@link ListableBeanFactory}和
 * {@link org.springframework.beans.factory.config.ConfigurableBeanFactory} 可用于特定用途。
 * <p>
 ...
 * 699/5000 这个接口是由包含多个bean定义的对象实现的 由字符串名唯一标识。根据bean定义，工厂 将返回一个包含对象的独立实例(原型设计)
 * 模式)，或单个共享实例(比单例设计更好的选择) 模式，其中实例是工厂范围内的单例)。哪种类型 实例的值取决于bean工厂的配置:API是 一样。从Spring
 * 2.0开始，根据具体情况提供了更多的作用域 应用环境(例如:“请求”和“会话”在web环境中适用)。
 * <p>
 ...
 * this approach.
 *
 * 这种方法的要点是BeanFactory是应用程序的中心注册表 组件，并集中配置应用程序组件(没有更多做 个别对象需要读取属性文件，例如)。见第4章和第11章
 * “专家一对一的J2EE设计和开发”的讨论 这种方法。
 *
 *
 * <p>
 ...
 * far as possible. The full set of initialization methods and their standard order
 * is:<br>
